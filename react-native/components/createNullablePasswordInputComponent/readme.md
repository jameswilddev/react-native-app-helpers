# `react-native-app-helpers/createNullablePasswordInputComponent`

Creates a new input component pre-configured as a nullable password input.

## Usage

```tsx
import { createNullablePasswordInputComponent } from "react-native-app-helpers";

const ExampleInput = createNullablePasswordInputComponent(
    {
...
const ExampleScreen = () => {
  ...
  const [incompleteValue, setIncompleteValue] = React.useState<undefined | null | string>(undefined);

  // Useful for persistence.
  const [completeValue, setCompleteValue] = React.useState<undefined | null | string>(undefined);
