...
test('allows a store to be loaded and read from', async () => {
  ...
  await store.load(randomUUID().toLowerCase())
...
test('treats two separate class instances as having their own state', async () => {
  ...
  await Promise.all([storeA.load(randomUUID().toLowerCase()), storeB.load(randomUUID().toLowerCase())])
...
test('throws an error when loading a loading store', async () => {
  ...
  void store.load(randomUUID().toLowerCase())

  const promise = store.load(randomUUID().toLowerCase())
