...
test('resubscribes when the event type changes', async () => {
  ...
  expect(event.removeListener).toHaveBeenCalledWith(
    'Event Type A',
    (event.addListener.mock.calls[0] as unknown[])[1]
  )
  expect(event.removeListener).toHaveBeenCalledWith(
    'Event Type B',
    (event.addListener.mock.calls[1] as unknown[])[1]
  )
...
test('resubscribes when the event type changes twice', async () => {
  ...
  expect(event.removeListener).toHaveBeenCalledWith(
    'Event Type C',
    (event.addListener.mock.calls[2] as unknown[])[1]
  )
...
test('resubscribes when the event changes', async () => {
  ...
  const Component: React.FunctionComponent<{
    readonly event: {
      addListener: (eventType: 'Event Type', listener: () => void) => void
      removeListener: (eventType: 'Event Type', listener: () => void) => void
    }
  }> = ({ event }) => {
    const invocations = React.useRef(0)
    invocations.current++
...
test('resubscribes when the event changes again', async () => {
  ...
  expect(eventC.removeListener).toHaveBeenCalledWith(
    'Event Type',
    (eventC.addListener.mock.calls[0] as unknown[])[1]
  )
