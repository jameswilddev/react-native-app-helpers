...
test(`executes the callback once when the ref is given, the layout is computed without a measure callback`, () => {
  let ref: React.RefCallback<View>;
  const measure = jest.fn();
  let onLayout: (event: LayoutChangeEvent) => void;
  const Component = () => {
    const [_ref, _onLayout] = useMeasure(undefined);
    ref = _ref;
    onLayout = _onLayout;
    return <View />;
  };
...
test(`does nothing when the layout is computed and the ref is given without a measure callback`, () => {
  let ref: React.RefCallback<View>;
  const measure = jest.fn();
  let onLayout: (event: LayoutChangeEvent) => void;
  const Component = () => {
